...
void Add_Particles(int n) {
    for(int i = 0; i < n; ++i) {
		...
		p.v = vec3(10 * p.x[0], random(1.0, 10.0), 10 * p.x[2]);
		p.color = vec3(255, 255, 0);
    	p.d = 0.0;
...
vec3 Get_Particle_Color(float d) {
	...
	if(d < 0.1)
		...
		return (1 - u1) * vec3(1, 1, 0) + u1 * vec3(1, 0, 0);
	else if(d < 2)
		return vec3(1, 0, 0);
	else if(d < 3)
		return (1 - u2) * vec3(1, 0, 0) + u2 * vec3(0.5, 0.5, 0.5);
	else
...
void application::draw_event()
{
    ...
    if (!paused) {
        ...
        for(unsigned i = 0; i < particles.size(); ++i) {
	    	...
	    	particles.at(i).f += vec3(0, -9.8 * particles.at(i).m, 0);
    ...
    glBegin(GL_LINES);
        ...
        for(unsigned i = 0; i < particles.size(); ++i) {
			particles.at(i).d += h;
	     	vec3 x0(particles.at(i).x + 0.04f * particles.at(i).v);
			vec3 color = Get_Particle_Color(particles.at(i).d);
